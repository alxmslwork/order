<?php
/**
 * Модуль для работы с профилем пользователя. Профили хранятся в шардовом хранилище. Деление пользователя на шарды
 *  происходит по конфигурации вручную для управления нагрузкой на хранилище (чтобы более активных пользвоатели могли
 *  получить больше ресурсов системы)
 */
...
/**
 * Функция обновления баланса пользователя. Обновление баланса осуществляется атомарной операцией UPDATE с инкрементом,
 *  поэтому вызовы данной функции возможны без внешней блокировки
 * @param int $userId идентификатор пользвоателя
 * @param float $money номинал полученных благ
